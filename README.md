...
- 将M3转M2橡胶减震柱穿过电驱和飞控的四角安装孔（技巧：橡皮筋技巧；对于电机板的减震柱，接触机架端的为长头（才保护接口），连接飞控减震柱的长头部分的为短头）

- 使用M2*20安装电驱（M2*16也可以，但螺丝不要拧太紧，能有回弹即可），8pin接口朝机头，接口贴着机架			

- 电机线修修剪（用剥线钳，裸线一定不要长，会炸板子），焊接(加松香)；电源线和滤波电容焊接（电源线先焊接接口，套上保护后再焊接到电机板）

- 连接飞控（见下面QCG版本问题），连接电池，更改电机序号映射和转向

-先把接收机和发射天线的线穿过镂空部分，把这几个模块都放到机架顶部，再插飞控板子的接口（插紧但小心飞控掉件）

- 连接接收机（遥传一体模块中靠近LED的是RC接口）到飞控的 RC 端口，数传（遥传一体模块中的另一个接口）到 TELEM1 端口，GPS模块连接 到 GPS 端口【安装无脑版：俩4p线，有按钮的接中间接口，可看Nxt-FC里的接口标号】

- ~~（略）M2\*12固定摄像头座，M2\*16固定图传天空端~~

- M3*6安装前六个铝柱的上部（不要旋紧），<font color="#dd0000">第七个在尾部的立柱先不装</font>（b站视频说错了，应该看三维动图）

- ~~（略）M2\*12安装配重打印件~~

- 3d打印天线座安装，大的套在机架尾部，小的套在涵道圈（大孔的是接触铝柱端），打印件有弹性，孔偏小，需要M3螺丝旋转着才能穿过

- <font color="#dd0000">把各个数据线的飞控端都装上（tele2（连接到orin的），gps，接收机）</font>

- 机架，七个铝柱，涵道圈，菱形底架，从上至下叠起来后从下往上安装M3*14

- 调试完成后安装桨叶，使用M2*8安装（建议使用较长螺丝钉旋转，找到下降处即为孔眼）
...
- 遥控器和接收机对频，并在地面站校准，并调遥控器的数传（具体看下述2.4的数传操作）

- 【注意：校准遥控器时候，“Flight Modes”校准按照下述操作】配置通道功能，用于飞行模式切换、紧急停止等（注意：有三档的前两档一致）详见 [px4ctrl](https://gitee.com/jerry-ironman/px4ctrl)
  - 5通道2档：自稳和offboard 
  - 6通道3档：是否接受命令 
  - 7通道3档：急停 
  - 8通道2档：px4ctrl对px4的重启
要出现黄色的Flight Modes，在选择“Stablilized”时。
...
### 2.4 遥控器数传模块
“五、1.数传设置”里MAV_0改成MAV_1，接口名称就是TELEM1不用改变。
